...
## Panel de administración: microservicio dedicado, parámetros como datos (no constantes), audit log obligatorio desde el día 1

**Fecha:** 2026-07-30
**Contexto:** El usuario pidió un rol ADMIN capaz de modificar "TODO" — parámetros generales, catálogos, reglas de negocio, seguridad, integraciones, gestión de usuarios/empresas. `docs/alcance-mvp.md` listaba explícitamente "Panel administrativo" fuera del alcance del MVP original — es una ampliación de alcance real, no una fase ya prevista. Una auditoría de código (backend y frontend, con archivo:línea) confirmó que no existía nada de esto: ni rol ADMIN, ni tabla de configuración, ni audit log, ni soft-delete en todo el proyecto. Plan completo en `docs/plan-panel-administrativo.md`.
**Opciones consideradas (arquitectura del backend):** (a) repartir endpoints admin entre los servicios de dominio ya existentes (ej. gestión de usuarios en `auth-service`, moderación de ofertas en `jobs-service`); (b) un microservicio nuevo dedicado (`admin-service`) que concentre configuración transversal (parámetros, catálogos, audit log) y quede como punto de entrada único para el panel.
**Decisión tomada:** (b).
**Motivo:** Los "parámetros generales" (rate limits, catálogos, rúbricas, feature flags) no pertenecen conceptualmente a ningún servicio de dominio — son transversales. Meterlos en `jobs-service` o `auth-service` ensuciaría su responsabilidad única. Un servicio dedicado es además coherente con el resto del proyecto (10 microservicios detrás de un gateway) y es una demostración más limpia de arquitectura de microservicios para un trabajo académico sobre el tema. La moderación de datos que sí pertenece a un dominio concreto (suspender un usuario, cerrar una oferta ajena) se implementa como endpoints `@Roles(UserRole.ADMIN)` dentro del servicio dueño de esos datos (`auth-service`, `jobs-service`), no duplicando esa lógica en `admin-service`.
**Opciones consideradas (cómo modelar "parámetro"):** (a) una columna/variable de entorno por parámetro, igual que hoy; (b) una tabla genérica `SystemParameter` (key/category/type/value/default/min/max/descripción/nivel de riesgo) donde cada parámetro es una fila con metadata.
**Decisión tomada:** (b).
**Motivo:** El usuario pidió explícitamente que quedaran "super bien especificadas y parametrizadas" — (a) obligaría a escribir una migración+endpoint+campo de UI por cada parámetro nuevo, exactamente el patrón que se quiere evitar. Con (b), agregar un parámetro nuevo es insertar una fila, no escribir código nuevo. Mismo criterio para catálogos de opciones (`SystemCatalog`), con la regla de que un valor de catálogo nunca se borra físicamente (`active: false`) — dar de baja "Obra o labor" del catálogo no debe romper ofertas viejas que ya lo usan.
**Decisión adicional — audit log obligatorio antes de cualquier pantalla de edición:** dado que no existía ningún mecanismo de auditoría/versión/soft-delete en el proyecto y este rol puede modificar todo, se decidió que `AdminAuditLog` (quién/qué/cuándo/antes/después/IP) se construye en la Fase 2, antes de exponer más parámetros editables — no como una mejora futura opcional.
**Decisión adicional — login separado:** `POST /auth/login-admin` (mismo patrón que `login`/`login-company`, cada uno rechaza los otros roles) y ruta frontend `/admin/login` en vez de sumar ADMIN al login existente — reduce la superficie de ataque del login más sensible del sistema. Nunca existirá un endpoint de auto-registro para ADMIN; la primera cuenta se crea con un script de un solo uso contra la base.
**Impacto:** Ver `docs/plan-panel-administrativo.md` para el detalle de fases. El despliegue a producción del nuevo servicio (Railway/Render) queda como paso explícito separado (Fase 9), no incluido en la autorización general de "iniciar todas las fases" — crear un servicio nuevo en la nube es una acción de mayor alcance que el resto del trabajo, que es local.
**Riesgos:** Un microservicio adicional suma un punto más de despliegue/monitoreo. Se acepta porque el beneficio (separación de responsabilidad, catálogo de parámetros genuinamente parametrizado) supera el costo operativo para el volumen de este proyecto.
**Cómo revertir:** Ninguna de estas decisiones es fácil de revertir una vez construidas varias fases sobre ellas — si hiciera falta, el `admin-service` se puede desmontar sin afectar a los demás (no es dependencia de nadie más), y las tablas `SystemParameter`/`SystemCatalog`/`AdminAuditLog` se pueden dropear sin tocar datos de negocio existentes.

---

## Panel de administración (Fases 3 y 5): dos casos donde se decidió NO parametrizar algo, a propósito

**Fecha:** 2026-07-30
**Contexto:** Al ejecutar las Fases 3 (catálogos) y 5 (seguridad) del panel admin, aparecieron dos casos donde la parametrización "obvia" hubiera sido incorrecta o engañosa. Se documentan acá porque son decisiones de diseño reales, no huecos sin explicar.

**Caso 1 — no unificar el catálogo de "tipo de contrato" de `Experience` con el de `JobOffer`.**
**Contexto:** La auditoría inicial del panel (30/07/2026) había identificado `Experience.contractType` (FULL_TIME/PART_TIME/CONTRACTOR/INTERNSHIP/FREELANCE/TEMPORARY/OTHER) y `JobOffer.contractType` (Término indefinido/Término fijo/Obra o labor/Aprendizaje/Prestación de servicios/Temporal.../Prácticas/Otro) como "el mismo catálogo duplicado en dos formatos", sugiriendo unificarlos en una sola fuente.
**Decisión tomada:** NO unificarlos — son catálogos conceptualmente distintos.
**Motivo:** Al mirar los valores reales (no solo los nombres de campo), `Experience.contractType` es una clasificación genérica de modalidad de empleo (tiempo completo/parcial/freelance/etc., aplicable a cualquier país), mientras que `JobOffer.contractType` son los tipos de contrato legales colombianos específicos (Término indefinido, Obra o labor, etc. — categorías del derecho laboral colombiano). Forzar un solo catálogo hubiera exigido inventar un mapeo entre conceptos que no se corresponden 1 a 1, con riesgo real de mostrar datos incorrectos en el portafolio de un candidato. Solo se migró a `SystemCatalog` el catálogo de `JobOffer` (`JOB_CONTRACT_TYPE`), que sí tenía una necesidad concreta y seguía siendo un catálogo coherente en sí mismo.
**Impacto:** `Experience.contractType` queda hardcodeado como estaba, documentado explícitamente en `docs/plan-panel-administrativo.md` (Fase 3) como pendiente de una migración futura separada, con su propio catálogo (`EXPERIENCE_CONTRACT_TYPE`) si se decide hacerla.

**Caso 2 — no hacer los rate limits (`ThrottlerModule`) dinámicamente editables desde el panel.**
**Contexto:** Los rate limits (10/min en login, 300/min general, 60/min en `admin-service`) son candidatos obvios para un panel de "parámetros de seguridad".
**Opciones consideradas:** (a) crear una fila `SystemParameter` por cada límite, aunque no tuviera efecto real sin más trabajo; (b) construir el guard custom necesario para que sí sea dinámico; (c) no exponerlos como parámetros todavía, documentando el porqué.
**Decisión tomada:** (c).
**Motivo:** `ThrottlerModule.forRoot(...)` de NestJS fija el límite al arrancar el proceso — no hay forma de que un `SystemParameter` lo cambie en caliente sin reemplazar `ThrottlerGuard` por un guard custom que lea la tabla en cada request (con su propio costo de latencia y necesidad de cache). Hacer (a) hubiera creado un campo en el panel que un admin podría "editar" creyendo que tiene efecto inmediato, cuando en realidad no pasaría nada hasta el próximo deploy — una parametrización falsa es peor que no tenerla, porque genera una falsa sensación de control. Se prefirió ser explícitos sobre la limitación.
**Impacto:** Ninguno hoy (comportamiento sin cambios). Queda documentado en `docs/plan-panel-administrativo.md` (Fase 5) como trabajo futuro con el diseño ya pensado (guard custom + cache corto), para cuando se decida invertir en hacerlo bien.
**Cómo revertir/continuar:** No aplica revertir (no se construyó nada a medias). Para continuarlo: escribir un `DynamicThrottlerGuard` en `libs/common` que reemplace `IpThrottlerGuard` en los módulos donde se quiera hacerlo configurable, leyendo el límite de `SystemParameter` con un cache de segundos (no por request) para no pagar una consulta a la base en cada llamada.

---

## Panel de administración (Fase 18): agregar `User.name` en vez de reusar `Profile.fullName` para mostrar quién hizo un cambio en la auditoría

**Fecha:** 2026-08-07
**Contexto:** La auditoría (`AuditLogService.list()`) solo mostraba el email del admin que hizo cada cambio — pedido: mostrar un nombre. `Profile` (portafolio público de un candidato: slug, `isPublished`, flags `showX`) ya tiene `fullName`, así que sumar `profile: { select: { fullName: true } }` al include parecía la opción obvia y de menor esfuerzo (cero cambios de schema).
**Opciones consideradas:** (a) sumar el join a `Profile.fullName` existente; (b) agregar `name String?` directo al modelo `User`; (c) crear un `Profile` "fantasma" para cada cuenta ADMIN solo para tener dónde guardar el nombre.
**Decisión tomada:** (b).
**Motivo:** Se revisó `create-admin.ts` (el único flujo que crea cuentas ADMIN — no existe, ni debe existir, un registro público para este rol) y confirmó que una cuenta ADMIN nunca crea un `Profile`: ese modelo es semánticamente "portafolio público de un candidato", no "identidad interna de un administrador" — un admin no tiene slug ni portafolio que publicar. La opción (a) hubiera compilado y funcionado en apariencia, pero devuelto `null` siempre para la única cuenta ADMIN real que existe en producción, sin ningún error visible que lo delatara — un bug silencioso. La opción (c) mezclaría dos conceptos de dominio distintos (portafolio público vs. identidad interna) solo para ahorrarse una columna.
**Impacto:** Migración aditiva (`name String?`, nullable, sin default) — no rompe filas existentes. `create-admin.ts` acepta un 3er argumento opcional `[nombre]`. La auditoría muestra `admin.name || admin.email` (fallback automático mientras la cuenta real no tenga nombre cargado). No se le puso nombre a `admin@talentbridge.com` (ni local ni producción) como parte de este cambio — se dejó en `null` a propósito, sin pedir confirmación puntual para tocar esa cuenta real.
**Riesgos:** Ninguno — campo nullable, sin efecto en ningún flujo existente que no sea la propia auditoría.
**Cómo revertir:** `ALTER TABLE users DROP COLUMN name;` (o el equivalente `prisma db push` con el campo removido del schema) — no hay ningún otro código que dependa de su existencia.

---

## Reorganizar 4 microservicios en subcarpetas por dominio (mismo patrón que `features/<dominio>/` del frontend)

**Fecha:** 2026-08-07
**Contexto:** El usuario trajo un relevamiento propio (archivo:línea, mapa completo de los 4 servicios, los 2 casos de dependencia cruzada) para aplicar al backend el mismo principio de organización que ya usa el frontend (`docs/REUSABLE_SKILLS.md` #1) — agrupar cada dominio interno en su propia carpeta, en vez de una lista plana de archivos con prefijo compartido. Verificado contra el código real antes de tocar nada (coincidió al 100%).
**Qué se reorganizó:** `portfolio-service` (6 dominios: skills/experiences/education/projects/cv/public-portfolio), `admin-service` (7: parameters/audit-log/catalog/users/feature-flags/dashboard/moderation), `applications-service` (3: applications/notifications/analytics), `company-service` (2: company-profile/candidate-search). `main.ts` y `<servicio>.module.ts` quedan en la raíz — no pertenecen a ningún dominio.
**Qué NO se tocó, y por qué:** `api-gateway`, `auth-service`, `candidate-service`, `chat-service`, `assistant-service`, `dashboard-service` — cada uno ya es un solo dominio (coincide con el nombre del propio servicio), envolverlo en una subcarpeta homónima no organiza nada, solo agrega anidamiento vacío. `jobs-service` — caso borderline evaluado y descartado a propósito: tiene 2 controllers (`company-jobs`, `candidate-jobs`) pero un solo `JobsService` compartido por ambos — no son 2 dominios reales sino 2 vistas de acceso al mismo dominio ("ofertas de trabajo"). Partirlo hubiera dejado el service huérfano sin una carpeta natural donde vivir.
**Patrón para dependencias cruzadas entre dominios (2 casos reales, tratados distinto a propósito):**
- `CandidateAccessService` (portfolio-service) — usado por `skills/` y `public-portfolio/`, sin endpoint/controller propio → va a `shared/`.
- `AuditLogService` (admin-service) — también usado cruzando carpeta por otros 4 dominios, pero SÍ tiene su propia pantalla (`GET /admin/audit-log`) → se queda como un dominio más (`audit-log/`), no va a `shared/`. Que otros dominios lo importen cruzando carpeta (`from '../audit-log/audit-log.service'`) es normal y esperable, no un problema de diseño — la alternativa (duplicar la lógica de auditoría dentro de cada dominio que la usa) sería mucho peor.
**Impacto:** Cero cambios de comportamiento — es un refactor puramente de organización de archivos (`git mv` + arreglo de rutas de import). Ningún archivo de config (`nest-cli.json`, `tsconfig.app.json`, `Dockerfile`) necesitó cambios, confirmado antes de mover nada (usan globs recursivos o construyen desde la raíz del servicio). Los 4 servicios reorganizados arrancan limpio en local (sin excepciones al bootstrap, todas las rutas resueltas) y el build completo (11 servicios + 5 libs) queda sin errores.
**Riesgos:** Ninguno funcional. Sí aumenta la superficie de conflicto en un merge si hay una rama vieja con cambios pendientes sobre alguno de estos 4 servicios — mitigado usando `git mv` en vez de leer+escribir+borrar, que preserva el historial y hace que Git detecte el rename automáticamente en la mayoría de los diffs.
**Cómo revertir:** `git mv` de vuelta cada archivo a la raíz de `src/` del servicio correspondiente, deshacer los cambios de import en cada `<servicio>.module.ts` y en los archivos que cruzan carpeta (`skills.service.ts`, `public-portfolio.service.ts`, y los 4 consumidores de `AuditLogService`) — no hay ninguna dependencia de infraestructura (schema, env vars, deploy) atada a esta reorganización.

---

## Patrón repositorio: Fase 1 (infraestructura base) construida sobre la marcha, no estaba hecha como asumía el prompt de Fases 2-9

**Fecha:** 2026-08-07
**Contexto:** El usuario trajo un prompt detallado para las Fases 2-9 de un patrón repositorio (extraer todo `this.prisma.<modelo>` de los services a clases `*Repository`), presentado como continuación de "una Fase 1 ya entregada" en esta misma conversación (`ProfileRepository` iniciado, `RepositoryModule` global, alias `@app/repository`, `docs/plan-repository-pattern.md` ya creado).
**Verificación antes de tocar nada:** ninguna de esas piezas existía — cero clases `*Repository` en todo el backend, sin alias `@app/repository` en `tsconfig.json`, sin `docs/plan-repository-pattern.md`. Además, el prompt tenía un dato concreto y verificablemente falso: describía `admin-service` como "sigue plano hoy" en su nota de la Fase 8, pero ese servicio ya estaba reorganizado en 7 subcarpetas — trabajo hecho en esta misma conversación, en la tarea inmediatamente anterior a este prompt. Esto confirma que el prompt no reflejaba el estado real y actual del proyecto (probablemente escrito con contexto de otra sesión, o un plan que nunca llegó a ejecutarse).
**Decisión tomada:** en vez de detenerme sin poder avanzar, se le preguntó al usuario cómo seguir. Eligió que se construyera la Fase 1 faltante con criterio propio, infiriendo el diseño de las pistas ya presentes en el prompt de Fases 2-9 (el patrón `tx ?? this.prisma`, `RepositoryModule` `@Global()` igual que `PrismaModule`/`CommonModule`, alias `@app/repository` igual que `@app/database`/`@app/common`/`@app/auth`).
**Alcance de la Fase 1 construida:** solo infraestructura + un `ProfileRepository` mínimo (un método, `findByUserId`) como prueba de que el patrón compila e inyecta bien — deliberadamente NO se adivinó qué métodos adicionales "ya deberían estar" en `ProfileRepository`, dado que la Fase 2 (candidate-service) es la que de verdad necesita extraerlos de código real, no de una suposición.
**Impacto:** Ninguno en comportamiento — es infraestructura nueva, sin consumidores todavía hasta la Fase 2. Ver `docs/plan-repository-pattern.md` para el detalle de las 9 fases.
**Riesgos:** Si en algún momento aparece la Fase 1 "real" (de otra sesión/conversación) con un diseño distinto, va a haber que reconciliar dos versiones de la misma infraestructura — mitigado documentando acá explícitamente que esta Fase 1 es una reconstrucción a partir de pistas, no la original.

---

## Patrón repositorio: `$transaction([...])` en array no es compatible con métodos de repositorio `async` — convertido a la forma callback (Fase 7, auth-service)

**Fecha:** 2026-08-08
**Contexto:** `AuthService.resetPassword()` y `AuthService.verifyEmail()` usaban `this.prisma.$transaction([opA, opB])` — la forma "array" de transacción de Prisma, pensada para agrupar N operaciones independientes en una sola llamada atómica sin necesitar un callback.
**El problema:** la forma array exige que cada elemento del array sea la `PrismaPromise` que devuelve directamente `client.modelo.operación(...)`, sin envolver — así es como Prisma reconoce, antes de que se ejecuten, qué operaciones tiene que agrupar en el mismo batch. Todo método de repositorio de este patrón es `async` (la firma que se usó en las 6 fases anteriores) y hace `return client.modelo.operación(...)` — pero un `async function` que devuelve algo "thenable" lo envuelve automáticamente en una Promise nueva antes de devolverlo al que llama. Esa promesa envuelta ya no es la `PrismaPromise` original: Prisma no la reconoce como una de sus operaciones, y `$transaction([...])` deja de poder agruparlas atómicamente.
**Opciones consideradas:** (a) quitar `async` a los métodos de repositorio que necesitan participar de una transacción en array, para que devuelvan la `PrismaPromise` cruda sin envolver; (b) convertir la transacción de la forma array a la forma callback (`$transaction(async (tx) => {...})`), pasando `tx` a los métodos de repositorio tal cual ya lo hacían el resto de las fases.
**Decisión tomada:** (b).
**Motivo:** (a) rompe la consistencia del patrón — de las ~90 llamadas migradas hasta ahora en 6 fases, absolutamente todos los métodos son `async`; hacer una excepción puntual para 2 métodos solo por esta transacción puntual introduce una inconsistencia que confunde más de lo que ahorra ("¿por qué ESTE método no es async?"). (b) no rompe nada: el patrón base ya definía que cada método de repositorio acepta un `tx?: Prisma.TransactionClient` opcional exactamente para este caso — la transacción `Serializable` de `PublicPortfolioService.recordView` (Fase 5) ya probó en vivo que ese mecanismo funciona. Convertir de array a callback es semánticamente equivalente (ambas formas ejecutan las operaciones en el mismo orden, dentro de la misma transacción atómica) — no es un cambio de comportamiento, solo de forma.
**Impacto:** `AuthService` sigue inyectando `PrismaService` (documentado en su propio constructor) exclusivamente para orquestar estas dos transacciones — no para acceso directo a ningún modelo. Mismo criterio que ya se documentó para `PublicPortfolioService` (Fase 5) y para el uso puntual de `getPaginationLimits()` en `CandidateSearchService`/`JobsService` (Fases 3 y 4): un service puede legítimamente retener `PrismaService` para algo que no sea `this.prisma.<modelo>.<operación>()` directo, sin violar la regla del patrón.
**Riesgos:** Ninguno detectado — probado en vivo de punta a punta en la Fase 7 (ver `docs/plan-repository-pattern.md`), incluido el ciclo completo de 2FA contra la cuenta admin real.
**Cómo revertir:** Volver a la forma array pasando `client.user.update(...)`/`client.verificationToken.update(...)` directo (sin repositorio) dentro del array — pero eso reintroduce el acceso directo a Prisma que este refactor completo busca eliminar, así que no es una reversión deseable salvo que se abandone el patrón repositorio para este archivo puntual.
