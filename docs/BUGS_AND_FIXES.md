...
**Prueba realizada:** `ng build`/`lint:css` limpios, sin warnings nuevos. Confirmado por lectura directa (solo lectura, sin escritura) contra la base de producción que la cuenta del usuario ya estaba `emailVerified: true` con cero tokens pendientes, confirmando que el bug era exclusivamente de estado en memoria del frontend, no del backend. Verificado en local que el banner sigue sin mostrarse (ni se dispara el refresco innecesario) para una cuenta ya verificada desde el arranque de la sesión. Este fix está incluido en el mismo commit pendiente de la Fase de verificación bloqueante (BUG-041) — no se ha desplegado todavía a producción, así que el usuario puede seguir viendo el banner viejo en su pestaña actual hasta que se despliegue (un `Ctrl+Shift+R`/recarga completa de la pestaña ahora mismo también lo esconde, sin esperar el deploy, porque el backend ya está correcto).
**Estado:** Corregido (pendiente de despliegue).
