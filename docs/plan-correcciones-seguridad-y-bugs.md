...
- [x] **1.1 — Sin rate limiting en ningún endpoint (incluido login).** No hay `@nestjs/throttler` ni equivalente instalado.
  ...
  - **Bug adicional encontrado en la verificación en vivo:** `ThrottlerModule` + `ThrottlerGuard` estándar funcionaban pegándole directo a `auth-service`, pero **no** disparaban a través del gateway (`req.ip`/`trust proxy` de Express no resolvía una IP consistente con el hop extra del proxy del gateway + el borde de Render). Corregido con un guard custom, `IpThrottlerGuard`, que lee la IP directamente de `X-Forwarded-For`. Ver detalle y evidencia de la reverificación en `docs/CHANGELOG.md` (commit `ea0a3b1`).
  - **Extendido a los otros 9 servicios (2026-07-26).** Estaba anotado como "evaluar si extenderlo" — se decidió que sí, tras encontrarlo listado como pendiente en el reporte de inspección del barrido de validadores. `IpThrottlerGuard` se movió a `BACKEND/libs/common/src/guards/` (antes vivía solo en `auth-service`) y se aplicó `ThrottlerModule.forRoot([{ name: 'default', ttl: 60000, limit: 300 }])` + `APP_GUARD` en los 9 módulos restantes (api-gateway y los 8 servicios de negocio). 300 req/min por IP — más laxo que el límite de login (10/min) a propósito, para no afectar uso normal. Verificado en vivo con carga concurrente real (`xargs -P 40`, 320 requests) contra el gateway y contra `dashboard-service`: exactamente 300 pasan, el resto devuelve 429. Ver `docs/CHANGELOG.md` para el detalle completo.
