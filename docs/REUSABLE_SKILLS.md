...
## 31. Al agregar un modelo/campo nuevo a Prisma mientras hay servicios NestJS corriendo en `--watch`, reiniciarlos a mano después de `prisma generate` — el watcher no lo hace solo

**Cuándo usarla:** Cada vez que se corra `npx prisma migrate deploy` + `npx prisma generate` (agregar un modelo, un campo, un enum) mientras uno o más microservicios ya están corriendo en modo desarrollo (`nest start --watch` o equivalente) desde antes de ese cambio.
**Síntoma:** Los endpoints que tocan el modelo/campo nuevo devuelven `500 Internal Server Error` genérico (el filtro de excepciones global oculta el error real) inmediatamente después de escribir el código que los usa, aunque el build (`npm run build:<servicio>`) haya compilado limpio y el código se vea correcto.
**Causa:** `nest start --watch` reinicia el proceso cuando cambia un archivo *fuente* (`.ts` bajo `apps/`/`libs/`), pero el cliente de Prisma regenerado vive en `libs/database/src/generated/client` — un directorio que el watcher no necesariamente vigila de la misma forma, y aunque lo reinicie, el proceso Node ya tenía ese módulo cargado en memoria desde su arranque original (el `require`/`import` de Node no revalida un módulo ya cacheado solo porque el archivo en disco cambió). El proceso sigue sirviendo requests con el cliente de Prisma *viejo*, que no conoce el modelo/campo nuevo — de ahí el 500 al intentar usarlo.
**Pasos:**
1. Escribir la migración + `npx prisma migrate deploy` + `npx prisma generate` como siempre.
2. Antes de probar en vivo, identificar qué servicios ya estaban corriendo *desde antes* de este cambio y que tocan el modelo/campo nuevo (no todos los 11 necesariamente — solo los que lo usan).
3. Matar esos procesos a mano (`netstat -ano | findstr :<puerto>` para el PID, `taskkill /PID <pid> /F` en Windows) y volver a arrancarlos (`npm run start:<servicio>`).
4. Recién ahí probar en vivo — un 500 después de este paso sí es un bug real de código, no de entorno.
**Errores comunes:** Asumir que como el build compiló limpio y el watcher "dice" que recargó (loguea `Starting Nest application...` de nuevo), el cliente de Prisma también se actualizó — el log de reinicio no distingue si fue un reinicio completo del proceso Node o algo más liviano.
**Checklist:** ¿El modelo/campo es nuevo desde la última vez que este servicio arrancó? Si sí, ¿se reinició el proceso a mano después de `prisma generate`, no solo se confió en el watcher?
**Ejemplo en TalentBridge:** Fase 12 del panel administrativo (2026-08-03) — el modelo `Report` nuevo causó `500` en `jobs-service`/`chat-service`/`admin-service` al crear/consultar reportes, con build limpio y código correcto. Diagnosticado reproduciendo el error, descartando causas de código, y confirmando que los 3 procesos venían corriendo desde antes de la migración — reiniciados a mano, resuelto de inmediato.

---

## 32. Organizar un backend NestJS de microservicios con varios dominios por servicio en subcarpetas — mismo principio que un frontend por `features/`, aplicado del otro lado

**Cuándo usarla:** Un microservicio backend acumula varios dominios internos reales (cada uno con su propio controller+service+DTO, ej. "skills", "experiencias", "educación" dentro de un mismo `portfolio-service`) y la carpeta `src/` queda como una lista plana de archivos que solo se distinguen por el prefijo del nombre — el mismo síntoma que ya resuelve `features/<dominio>/` del lado del frontend (ver skill #1), pero nadie lo aplicó todavía del lado del backend.
**Pasos:**
1. Agrupar cada conjunto de archivos que comparte dominio (`X.controller.ts` + `X.service.ts` + su/sus DTO) en `src/<dominio>/`, sin renombrar nada — el prefijo del nombre de archivo no se pierde por estar ya dentro de una carpeta con ese nombre (mismo criterio que `features/admin/admin-users.component.ts` en el frontend).
2. Si hoy existe una carpeta `dto/` plana separada, los DTO se mueven DENTRO de la carpeta de su dominio, no quedan en su propia subcarpeta `dto/` — un dominio con un solo DTO no justifica un nivel más de anidamiento.
3. `main.ts` y `<servicio>.module.ts` se quedan en la raíz de `src/` — no pertenecen a ningún dominio en particular, y `main.ts` no necesita ningún cambio (sigue importando el módulo desde `./`).
4. En `<servicio>.module.ts`, actualizar solo las rutas de import de controllers/services al nuevo path — el resto (providers de throttling, `APP_GUARD`/`APP_PIPE`/`APP_FILTER`) no cambia.
5. Usar `git mv` para cada archivo (nunca leer+escribir+borrar) — preserva el historial y Git lo detecta como rename en el diff, no como un archivo borrado + uno nuevo.
6. Antes de mover nada, confirmar que `nest-cli.json`, `tsconfig.app.json` y el `Dockerfile` no listan archivos explícitos (usan globs recursivos o construyen desde la raíz del servicio) — si alguno sí lista rutas a mano, hay que actualizarlo también.
7. Un servicio a la vez, con build después de cada uno (`npm run build:<servicio>`) — si algo rompe, el radio de daño queda acotado a un solo servicio en vez de los N que se estén tocando de una.
**Caso especial — dependencia cruzada entre dos dominios sin un tercer dominio natural:** si un service lo usan dos o más dominios distintos y no tiene su propio controller/endpoint, va a `shared/` (ni al dominio A ni al B). Si en cambio SÍ tiene su propio endpoint (una pantalla propia en el frontend), se queda como un dominio más aunque otros lo importen cruzando carpeta — eso es normal y esperable, no un error de diseño. Distinguir estos dos casos evita terminar con una carpeta `shared/` que en realidad es "todo lo que no supimos dónde poner".
**Cuándo NO aplicarla:** Un servicio que ya es un solo dominio (coincide con el nombre del propio servicio) no gana nada envolviéndose en una subcarpeta homónima — solo agrega un nivel de anidamiento vacío. Tampoco aplica a un servicio con 2+ controllers que en realidad comparten un solo service (2 vistas de acceso al mismo dominio, no 2 dominios reales) — partirlo dejaría el service huérfano sin una carpeta natural.
**Errores comunes:** Mover archivos con Read+Write+Bash(rm) en vez de `git mv` — Git ve un archivo borrado y uno nuevo sin relación, se pierde el historial de line-blame de cada línea. Olvidar que un import `./dto/X.dto` pasa a `./X.dto` (no `../dto/X.dto`) porque el DTO viaja CON su controller/service a la misma carpeta, no se queda atrás.
**Checklist:** ¿Cada `git mv` preservó el historial (`git status` muestra `renamed:`, no `deleted:`+`new file:`)? ¿El único import que cambió en cada archivo movido es el de su propio DTO y el de dependencias cruzadas reales? ¿Build service-por-service antes del build completo? ¿Los servicios reorganizados arrancan sin excepciones (no solo compilan)?
**Ejemplo en TalentBridge:** 2026-08-07 — `portfolio-service`, `admin-service`, `applications-service`, `company-service` reorganizados en 6+7+3+2 dominios respectivamente, con 2 casos de dependencia cruzada resueltos distinto (`CandidateAccessService` → `shared/`, `AuditLogService` → dominio propio `audit-log/` con imports cruzados desde los otros 4). Los 11 servicios compilaron limpio (`npm run build`) y los 4 reorganizados arrancaron sin excepciones en local antes de dar la tarea por cerrada. Ver `docs/DECISIONS.md` para el detalle completo de la decisión.
