# Dockerfile de deploy para company-service (Railway) — build aislado, solo este servicio.
...
RUN npx nest build company-service

FROM node:22-alpine
WORKDIR /app
COPY --from=build /app/node_modules ./node_modules
COPY --from=build /app/dist/apps/company-service ./dist
