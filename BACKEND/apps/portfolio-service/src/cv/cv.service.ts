...
import { CvDocumentRepository, CvAnalysisRepository, SystemParameterRepository } from '@app/repository';
...
export class CvService {
  ...
  constructor(
    private readonly cvDocumentRepository: CvDocumentRepository,
  ...
  private async getMaxPdfSizeMb(): Promise<number> {
    const param = await this.systemParameterRepository.findByKey('MAX_PDF_SIZE_MB');
    const value = Number(param?.value ?? process.env['MAX_PDF_SIZE_MB'] ?? 5);
    return Number.isFinite(value) && value > 0 ? value : 5;
  }

  async getCvs(userId: number) {
    return this.cvDocumentRepository.findByUserId(userId);
  }

  async getOne(userId: number, cvId: number) {
    const cv = await this.cvDocumentRepository.findByIdAndUserIdWithAnalyses(cvId, userId);
  ...
  async uploadCv(userId: number, file: Express.Multer.File) {
    ...
    return this.cvDocumentRepository.create({ userId, originalName: file.originalname, filePath, mimeType: file.mimetype, extractedText });
  }

  async analyzeCv(userId: number, cvId: number) {
    const cv = await this.cvDocumentRepository.findByIdAndUserId(cvId, userId);
    if (!cv) throw new NotFoundException('CV no encontrado');
    ...
    if (!text && cv.filePath && fs.existsSync(cv.filePath)) {
      try {
        ...
        if (text) {
          await this.cvDocumentRepository.update(cvId, { extractedText: text });
        }
      } catch {
  ...
  async getAnalyses(userId: number, cvId: number) {
    ...
    return this.cvAnalysisRepository.findByCvDocumentId(cvId);
  }
  ...
  async deleteCv(userId: number, cvId: number) {
    ...
    await this.cvDocumentRepository.delete(cvId);
  ...
  private async getRubricWeights(): Promise<CvRubricWeights> {
    const param = await this.systemParameterRepository.findByKey('CV_RUBRIC_WEIGHTS');
...
Responde ÚNICAMENTE con un objeto JSON con esta forma exacta, sin texto antes ni después (recuerda: TODO EN ESPAÑOL):
{
  ...
  "recommendations": array de 3 a 5 strings cortos EN ESPAÑOL, cada uno una recomendación ACCIONABLE y específica para mejorar este CV puntual
    ...
    return this.cvAnalysisRepository.create({ cvDocumentId: cvId, score, strengths, recommendations });
  }
}
